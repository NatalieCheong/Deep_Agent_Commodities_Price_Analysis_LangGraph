...
class CommodityAnalysisAgent:
    ...
    def data_collection_node(self, state: AgentState) -> AgentState:
        ...
        try:
            # Get the list of commodities to analyze from state
            commodities_to_fetch = state.get("commodities_to_analyze", [])
            
            if not commodities_to_fetch:
                commodities_to_fetch = Config.get_commodity_list()
            
            # Fetch only the specified commodities
            all_data = {}
            for commodity in commodities_to_fetch:
                try:
                    print(f"Fetching {commodity}...")
                    data = self.data_collector.fetch_commodity_data(commodity)
                    
                    if not data.empty:
                        all_data[commodity] = data
                        print(f"  ✓ Fetched {len(data)} records for {commodity}")
                    else:
